...
export interface Tile {
  ...
  rich?: boolean;
  /**
   * 今の持ち主が、取引（土地提案の承諾）でこのマスを手に入れたかどうか。
   * trueのマスは、開拓の隣接判定の起点として使えない（trueでなくなるまで、
   * このマスから隣に広げることはできない）。開拓・橋・奪う・着工で手に入れた
   * マスは常にfalseになる。
   */
...
export type Command =
  ...
  | {
       ...
       * 名前（例: "灯台"）でどれに出すかを指定する必要がある
       * （1つしか進行していなければ省略できる。同時進行中に名前が
       * 被ることは無いので、名前だけで一意に決まる）。
       */
      type: "export";
      player: string;
      resource: Resource;
